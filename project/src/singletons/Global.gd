...
func _ready() -> void:
	...
	Global.info(self, "screen width=%s, screen height=%s." %[screen_width, screen_height])
	VisualServer.set_default_clear_color(Color(0, 0, 0.5, 0))
...
func findNodeByName(name: String, root: Node = get_tree().current_scene):
